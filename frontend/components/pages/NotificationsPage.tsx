...
import { getNotificationHref, notificationClasses, formatRelativeTime } from "@/lib/claimUi";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? <div className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)]"><SkeletonBlock className="h-5 w-44" /><SkeletonBlock className="mt-3 h-4 w-3/4" /></div> : filtered.map((notification, index) => (
          <motion.div key={notification.id} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.24, delay: index * 0.05 }} onClick={() => markNotificationRead(notification.id)} className={`cursor-pointer rounded-[1.5rem] border border-slate-200 border-l-4 p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] transition-all hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(15,23,42,0.08)] sm:p-5 ${notificationClasses[notification.type]} ${notification.read ? "bg-white" : "bg-white/90"}`}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <p className="font-semibold text-slate-900">{notification.title}</p>
                <p className="mt-2 text-sm leading-6 text-[var(--ch-muted)]">{notification.message}</p>
                <p className="mt-3 text-xs text-[var(--ch-subtle)]">{formatRelativeTime(notification.time)}</p>
              </div>
